...
Exporting calculation files is step one, the other one is a bit more tempting.. reading existing files and converting them to my ```CalculationModel```. For this I had to dive into the geolib code and here is some code that might help other people working on the same problem. The code is off course not complete because it only shows part of the process but again, it might help on your geolib journey. I have added some extra comments which I hope makes things more clear.
